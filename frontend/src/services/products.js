...
export const createProduct = (formData) => api.post('/products', formData).then((response) => response.data.data.product);
export const updateProduct = (id, formData) => api.patch(`/products/${id}`, formData).then((response) => response.data.data.product);
